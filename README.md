...
4. **不联网、不抓取外部数据** —— 同一输入永远产生同一输出，可复现性优先
...
| 模块 | 状态 | 说明 |
|---|---|---|
| [01-data-intake](modules/01-data-intake/) | **[Released v0.2]** | 原始数据 → analysis-ready 数据集 + 数据合同 + 路由建议 |
...
仅含 `01-data-intake` 模块（v0.2）。pipeline 框架（契约规范、安装脚本、文档结构）就位，等待陆续加入 02-10 模块。
