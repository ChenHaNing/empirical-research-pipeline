...
### 原则 4：模块自主运行不联网、不调外部模型

模块在执行过程中**禁止**：

- 自主从网上获取任何资源（数据值、文献元数据、API 文档、BibTeX 等）
- 调用 LLM（包括 Claude 自身的训练知识做推断）
- 调用其他 skill 或 MCP

原因：

- **可复现性** —— 同一份原始数据 + 同一组用户回答必须永远产生同一份输出。任何外部依赖都会让今天跑和明年跑结果不同
- **可引用性** —— 论文脚注里写的数据来源必须是稳定可引的（文件号、文献年份），不能是某个 API 在某个时间点返回的值
- **离线运行** —— 研究者经常在本地 / 私网环境工作，模块必须能离线跑全部流程
- **审计简洁** —— 没有外部调用 = 没有需要审计的外部依赖 = 模块行为完全确定

如果某个步骤需要外部信息（例如查 ICD-10 代码定义、查方法学惯例），模块只**给出权威来源指针**写入 `unresolved_decisions`，由**研究者自己**离开 skill 去查询、决定、再回来更新数据。这样 skill 永远停在它该停的边界上。

**v0.3 的数据评价模块（data_evaluation）严格遵守此原则**——它仅基于 intake 已经算好的合同字段（行数、缺失率、相关性、outlier 数等），用 23 条确定性规则给出强项 / 弱项 / 综合评级，全程零外部调用、零 LLM 调用。详细见 `modules/01-data-intake/references/02-data-evaluation.md`。
