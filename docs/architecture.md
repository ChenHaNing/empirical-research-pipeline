...
模块**禁止**在执行过程中请求外部网络资源（行业代码字典、政府数据、维基百科等）。原因：

- **可复现性** —— 网络资源会变，今天跑和明年跑可能输出不一样
- **可引用性** —— 研究者发论文需要可引的数据来源；自动抓取的数据无法在脚注里列出权威来源
- **离线运行** —— 研究者经常在本地 / 私网环境工作，模块必须能离线跑

如果某个步骤需要外部数据（比如查 ICD-10 代码定义），模块只**给出权威来源指针**写入 `unresolved_decisions`，不替用户去查。
